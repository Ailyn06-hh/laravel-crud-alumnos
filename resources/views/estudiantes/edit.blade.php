...
<div class="max-w-md mx-auto mt-12 bg-white p-8 rounded-2xl shadow-lg border border-pink-100">
    <h2 class="text-2xl font-bold text-pink-700 mb-6 text-center">Editar Estudiante</h2>

    <form action="{{ route('estudiantes.update', $estudiante) }}" method="POST" class="space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="semestre" value="{{ old('semestre', $estudiante->semestre) }}"
                   class="w-full border border-pink-200 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-pink-400"
                   required>
        </div>
